...
**GWP value-set registry — the `ipcc:` vocabulary** (`extensions/ipcc-gwp.ttl`, `extensions/comet-pcr.ttl`, `tools/schemas/comet-pcf.schema.json`, `docs/gwp-value-sets.md`)
- Replaces the under-determined `ipccAR` Enum(AR5, AR6) with a value-**set** model keyed on IPCC edition **plus variant**, not edition alone. A GWP characterisation factor is only interpretable if you know which value set produced it, and an edition label cannot separate AR5 no-feedback / with-feedback / the UNFCCC 7/CP.27 variant, nor AR6 fossil / biogenic methane, nor express AR4 or SAR — both of which are live in regulation (California ARB inventory runs on AR4; US EPA GHGRP RY2010–RY2023 is an AR4 archive) and permanently embedded in tradeable credit stock (pre-2021 VCUs; transitioned SAR-basis CERs).
- New COMET-maintained controlled vocabulary `ipcc:` (namespace `https://comet.carbon/vocab/ipcc-gwp#`), **22 terms**: the `ipcc:valueSetScheme` SKOS scheme, the `ipcc:GWPValueSet` class, **7 value sets** (`SAR`, `AR4`, `AR5-noFeedback`, `AR5-withFeedback`, `AR5-UNFCCC`, `AR6-fossilCH4`, `AR6-biogenicCH4`) each carrying sourced witness GWP-100 values (CH₄, N₂O + the CBAM-statutory CF₄/C₂F₆), and the structural + relation properties (`edition`, `variant`, `publicationYear`, `pactExportable`, `pactEnumValue`, `hasGWP`/`forGas`/`gwp100`, and the machine-checkable `differsFrom`/`onGas`, `supersedes`, `isFloor`).
- **4 provenance properties on `comet-ef:GWP100Value`** (in `comet-pcr:`, candidate to upstream onto `comet-ef`): `gwpValueSet` (CURIE into `ipcc:`, null-safe = indeterminate), `arBasis` (SKOS scheme `Declared`/`BackSolved`/`Inferred`/`Indeterminate`), `arConfidence` (decimal [0,1], required when inferred), `gwpHorizon` (`GWP100`/`GWP20`), plus `informedByRule` → `comet-ver:DataSubstitutionRule` so a verifier sees which rule assumed a basis and with what confidence. `comet_pcr_pending` 36 → 51.
- `extensions/comet-pcr-shapes.ttl` SHACL shapes on `comet-ef:GWP100Value` (value-set membership, arBasis membership, confidence range, and two SPARQL constraints: inferred ⇒ confidence required, indeterminate ⇒ no value set) validated with pyshacl against `extensions/examples/gwp-provenance-valid.ttl` (Conforms: True) and `gwp-provenance-invalid.ttl` (Conforms: False on exactly 3 violations).
- **Export layer enforces the BLOCK** (`tools/converters/comet_to_pact.py`): `gwpValueSet` collapses to PACT's `characterizationFactors` edition where one exists; SAR, AR4 and any indeterminate factor are **non-exportable** and the field is omitted with a warning rather than asserting a false edition. Import (`pact_to_comet.py`) upgrades PACT's lossy edition tag to that edition's conventional value set (AR6 → `AR6-fossilCH4`, AR5 → `AR5-noFeedback`) with `arBasis = declared`. COMET conformance and PACT conformance therefore legitimately diverge — the ontology describes the world faithfully; the export layer enforces what each destination accepts.
- Schema: `ipccAR` marked **deprecated** (retained for PACT interoperability only); `gwpValueSet`/`arBasis`/`arConfidence`/`gwpHorizon` added with conditional-required logic (inferred ⇒ `arConfidence`; indeterminate ⇒ no `gwpValueSet`). Examples `steel-pcf` and `aluminium-pcf` updated. Registry total 539 → 576 CURIEs (`ipcc_pending` 22). Full rationale, worked examples and the back-solve method in `docs/gwp-value-sets.md`. `test/test_registry.py` extended (value sets present, ipcc terms TTL-defined, PACT BLOCK behaviour).

**I-REC(E) extension** (`ext/irec-e/`)
- New extension module `comet-ext:irec-e` (`comet-irec:`, namespace `https://comet.carbon/ext/irec-e#`) mapping the **Evident I-REC Code for Electricity v1.13** (EC-I-REC(E), 12 Jun 2025) and its Standard Forms **SF-02 Production Facility Registration v1.3** (+A/B/C) and **SF-04 Issue Request v1.2** (+A/B/C) onto the COMET seven-layer stack. 84 `comet-irec` terms: **19 classes**, **18 object properties**, **34 data properties**, **12 named Standard-Form individuals** (`SF-02`…`SF-07`)
- **Reuse-first**: an I-REC(E) certificate is a `comet-eac:EnergyAttributeCert` (subType `IREC`, already enumerated in `comet-eac.schema.json`); Registrant/Participant/Owner → `comet:Organization`; Production Facility/Installation → `comet:Site`; Production Period → `comet:TimePeriod`; registry → `comet-eac:CertificateRegistry`; redemption → `comet-eac:RetirementEvent`; auditor/agent → `comet-ver:QualifiedVerifier`. Reuse bridges recorded in `ext/irec-e/alignments/comet-irec-evident-alignment.ttl`
- New classes cover the I-REC-specific machinery with no prior COMET home: the SF-04 `IssueRequest`, the SF-04C `FuelConsumptionStatement`/`FuelInputRecord` co-firing allocation (`Fuel Consumed = b + c − d`; `Energy from Source = a × (b + c − d)`), the Code s.10 `RedemptionStatement` (QR code + verification key for account-less verification), `Account` (Trade/Redemption), and the anti-double-counting `IssuingDeclaration`/`RegistrantDeclaration`/`OwnerDeclaration`. Key `carbonOffsetRightsExcluded` flag and `chainOfCustodyReference` traceability backbone modelled per Code s.9
- `ext/irec-e/comet-ext-irec-e-shapes.ttl` SHACL shapes (request-type/evidence-type/account-type/public-funding enums, cardinality, and two SPARQL constraints enforcing the SF-04C arithmetic + `amountAppliedForMWh ≤ totalProductionMWh`) validated with pyshacl against the worked 45 MW biomass+coal co-firing example (`tests/data/example-irec-valid.ttl` Conforms: True; `example-irec-invalid.ttl` Conforms: False on 9 violations)
- New converter `tools/converters/irec_to_comet.py` (full; +`tools/examples/irec-input.json`) turning an SF-04-shaped Issue Request JSON into a COMET JSON-LD certificate, computing renewable-eligible MWh from the SF-04C allocation; wired into `tools/comet_cli.py` (`comet convert --from irec`). Output validates against `comet-eac.schema.json`. Test suite `tools/converters/tests/test_irec_converter.py` (allocation math, single-fuel path, registry/TTL consistency — all passing)
- Registered in `registry/namespaces.json`, `registry/comet-curies.json` (84 `comet_irec_pending` CURIEs; `counts.total` 451 → 535) and `comet-context.jsonld`
